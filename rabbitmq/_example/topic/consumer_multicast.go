...
func main() {
	...
	connection.Use(middlewares.QueueBind(queueName, "campaign.multicast.*", exchangeName, false, nil))
	if err := connection.Run(); err != nil {
		log.Panic(err)
	}

	consumer := rabbitmq.NewConsumer(queueName, "consumer_multicast", connection)
